...
import ConditionBuilderTab from './ConditionBuilderTab';
...
const CreateTemplatePage: React.FC = () => {
  ...
  const [conditionGroups, setConditionGroups] = useState<EmailTemplate['conditionGroups']>([]);
  ...
  const variableNames = useMemo(
    () => dynamicVariables.map(v => `{${v.variableName}}`),
    [dynamicVariables]
  );

  const conditionGroupNames = useMemo(
    () => (conditionGroups ?? []).map(g => `{{${g.name}}}`),
    [conditionGroups]
  );
  ...
  useEffect(() => {
    if (existingTemplate) {
      ...
      setConditionGroups(existingTemplate.conditionGroups || []);
      dispatch(setCurrentTemplate(existingTemplate));
      setEnabledTabs(['creation', 'variables','conditions', 'design']);
    }
  ...
  const handleNextFromVariables = () => {
    ...
    setEnabledTabs(['creation', 'variables','conditions']);
    setActiveTab('conditions');
  };

  const handleNextFromConditions = () => {
    if (isViewing) return;
  ...
  // handler to add a new condition group
  ...
  const handleConditionGroupsChange = (groups: EmailTemplate['conditionGroups']) => {
  ...
  const tabs = [
      ...
      content: (
        <div className="space-y-4">
          <ConditionBuilderTab
            ...
            fields={dynamicVariables.map(v => ({ label: v.variableName, value: v.variableName }))}
            isViewing={isViewing}
    ...
    },
    {
      ...
      content: <TemplateDesignStep isEmailTemplate={isEmailTemplate} isViewing={isViewing} variables={variableNames} conditions={conditionGroupNames} />,
    },
  ];
