...
const FilePreview: React.FC<{ attachment: FileAttachment }> = ({ attachment }) => {
  ...
  return (
    <>
      <div className="mt-2 p-2 bg-gray-50 rounded-lg border border-gray-200 text-sm dark:bg-gray-800 dark:border-gray-700">
  <div className="flex items-center justify-between p-2 rounded-md transition-colors">
    <div className="flex items-center space-x-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="font-medium text-gray-900 truncate dark:text-gray-100">{attachment.name}</div>
    ...
    <div className="ml-4 flex-shrink-0 flex items-center space-x-2">
      {canPreview && (
        <button
          onClick={handlePreviewClick}
          className="text-gray-500 hover:text-blue-600 p-1 rounded-full hover:bg-gray-100 transition-colors dark:text-gray-400 dark:hover:text-blue-400 dark:hover:bg-gray-700"
        >
          <Eye className="h-5 w-5" />
        </button>
      )}
